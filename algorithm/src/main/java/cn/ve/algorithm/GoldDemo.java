...
public class GoldDemo {
    ...
    public static void main(String[] args) {
        // 递归法获得最大收益
        int maxProfit = getMaxProfit(g.length, 10);
     ...
     * @return
     */
    private static int getMaxProfit(int n, int k) {
        // 没有金矿或没有工人，返回收益0
        if (n <= 0 || k <= 0) {
            return 0;
        }
        if (k < p[n - 1]) {
            return getMaxProfit(n - 1, k);
        }
        // 分解问题，每个金矿要么挖，要么不挖，从最后一个金矿开始
        // 1.如果挖最后一个金矿,收益就是最后一个金矿挖了，加上递归其他金矿
        // 如果挖最后一个金矿,收益由两部分组成 1.1.当前收益 1.2.其他金矿收益
        int profit1 = g[n - 1] + getMaxProfit(n - 1, k - p[n - 1]);
        // 2.如果没有挖最后一个金矿就直接排除最后一个金矿
        int profit2 = getMaxProfit(n - 1, k);
        // 上述1与2取最大收益返回
